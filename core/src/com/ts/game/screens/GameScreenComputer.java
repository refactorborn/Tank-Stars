...
public class GameScreenComputer extends ScreenAdapter implements Gameplay{
    //Scene2D Variables
    private Stage stage;
    ...
    public GameScreenComputer(final ts game) throws FileNotFoundException {
        //Overlaying Instantiations
        game.controls.resetGameControl();
        res = Resources.getGameResources();
        this.game = game;
        game.controls.parseScriptData();
        ...
        inputMultiplexer.addProcessor(new InputController(){
            ...
            public boolean touchDown(int screenX, int screenY, int pointer, int button) {
                ...
                if(game.controls.getCurrentPlayer()==0){
                    if(screenX>=playerTankBody.getPosition().x-100-Xoffset && screenX<=playerTankBody.getPosition().x+100-Xoffset && screenY+80>=playerTankBody.getPosition().y-100-Yoffset && screenY+80<=playerTankBody.getPosition().y+100-Yoffset){
    ...
    public void handleComputerGameplay(){
        if(currentScriptLine<game.controls.getScriptDataSize()) {
            System.out.println("Reading Script");
            String currentControls = game.controls.getScriptDataAtIndex(currentScriptLine);
            currentScriptLine++;
            String[] coordinates = currentControls.split(",");
            for (int i = 0; i < coordinates.length; i++) {
                if (coordinates[i].equals("L")) {
                    enemyTankBody.applyLinearImpulse(-speed, 0, enemyTankBody.getWorldCenter().x, enemyTankBody.getWorldCenter().y, true);
                } else {
                    enemyTankBody.applyLinearImpulse(speed, 0, enemyTankBody.getWorldCenter().x, enemyTankBody.getWorldCenter().y, true);
                }
            }
            while(game.controls.isShotActive()){}
            takeTurn(); // Creating Bullet
        }
    }
    ...
    public void scene2dCreateGround(){
        ...
        changeWpn = new TextButton(btnText, res.getTextButtonStyle2());
    ...
    public void box2dCreateTank(int player){
        ...
        if(player==0) {
            ...
            //Wheel1
            tankWheel.setPosition(new Vector2(-18, -15));
    ...
    public void handleHealth(){
        if(game.controls.getPlayer1Health()==9){
        ...
        if(game.controls.getPlayer2Health()==9){
    ...
    public void handleMutilation(int Xcoord, int YCoord){
        try{
            ...
            System.out.println("Error in handleMutilation");
        }
    }
    ...
    public void render(float delta) {
        ...
        if(!game.controls.isWpnWindowActive()) {
            if(game.controls.isCollisionDetection()){
                handleMutilation(game.controls.getXcollisionCoord(),game.controls.getYcollisionCoord());
        ...
        if(game.controls.getCurrentPlayer()==1 && !game.controls.isShotActive()){
            handleComputerGameplay();
        }
